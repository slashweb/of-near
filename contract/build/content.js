...
let Content = (_dec = NearBindgen({}), _dec2 = view(), _dec3 = call({}), _dec(_class = (_class2 = class Content {
...
function set_greeting() {
  const _state = Content._getState();
  if (!_state && Content._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = Content._create();
  if (_state) {
    Content._reconstruct(_contract, _state);
  }
  const _args = Content._getArgs();
  const _result = _contract.set_greeting(_args);
  Content._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Content._serialize(_result, true));
}
